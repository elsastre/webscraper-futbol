...
function App() {
  ...
  const [errorGoleadores, setErrorGoleadores] = useState("");
  ...
  const fetchGoleadores = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/goleadores?top=20`);
      if (!res.ok) {
        throw new Error("No se pudo obtener la tabla de goleadores.");
      }
      const data = await res.json();
      setGoleadores(data);
    } catch (err) {
      console.error(err);
      setErrorGoleadores(err.message);
    } finally {
  ...
  function handleChangeView(view) {
    ...
    if (view === VIEWS.TEAMS && teams.length === 0) {
      ...
      loadAttacks();
    } else if (view === VIEWS.GOLEADORES && goleadores.length === 0 && !loadingGoleadores) {
      fetchGoleadores();
    }
  }
  ...
  function renderGoleadoresTable() {
    if (!goleadores || goleadores.length === 0) {
      return (
        <p style={{ color: "#9ca3af" }}>
          No hay datos de goleadores. Refrescá los standings primero.
        </p>
      );
    }
          ...
          <tbody>
            {goleadores.map((goleador, index) => (
              <tr key={index} style={trStyle}>
                ...
                <td style={tdStyle}>{goleador.Jugador}</td>
                <td style={tdStyle}>
                  <TeamName name={goleador.Equipo} />
                </td>
                <td style={tdStyle}>{goleador.Goles}</td>
              </tr>
            ))}
          </tbody>
        </table>
    ...
    >
      ...
      >
        ...
        >
          ...
          <TabButton
            label="Goleadores"
            active={activeView === VIEWS.GOLEADORES}
            onClick={() => handleChangeView(VIEWS.GOLEADORES)}
          />
          <TabButton
            label="Buscar equipo"
        ...
        <main>
          ...
          {activeView === VIEWS.GOLEADORES && (
            <section>
              <h2 style={{ marginBottom: "1rem", fontSize: "1.5rem" }}>Tabla de Goleadores</h2>

              {errorGoleadores && (
                <p style={{ color: "#fb923c", marginBottom: "0.75rem" }}>{errorGoleadores}</p>
              )}

              {loadingGoleadores && (
                <p style={{ marginBottom: "0.75rem" }}>Cargando goleadores...</p>
              )}

              {!loadingGoleadores && !errorGoleadores && renderGoleadoresTable()}

              {!loadingGoleadores && !errorGoleadores && goleadores.length === 0 && (
                <p style={{ color: "#9ca3af" }}>
                  No hay datos de goleadores para mostrar. Refrescá los standings primero.
                </p>
              )}
            </section>
          )}
